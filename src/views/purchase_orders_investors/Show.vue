        ...
        <v-col cols="12">
          <v-card>
            <v-card-title>
              <v-row dense>
                <v-col cols="11">
                  <CardTitle :text="'DATOS GENERALES | ' + item.id" sub />
                </v-col>
            ...
            <v-card-text>
              <v-row dense>
                <v-col cols="12" md="6">
                  <VisVal label="Inversionista" :value="item.user.name + ' ' + item.user.paternal_surname" />
                </v-col>

                <v-col cols="12" md="6">
                  <VisVal label="Porcentaje" :value="item.investor_companies[0].floor_percentage + '%'" />
                </v-col>
              </v-row>
            </v-card-text>
          </v-card>
...
const restoreItem = async () => {
  ...
  isLoading.value = true;
  try {
    //  const endpoint = `${URL_API}/purchase_orders/${purchaseOrderId.value}/purchase_order_investors/restore`;
    //  const response = getRsp(
    //    await axios.post(
    //      endpoint,
    //      { id: itemId.value },
