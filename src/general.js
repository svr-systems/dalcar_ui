...
//función arma dinámicamente los headers para que puedas usarla en peticiones HTTP, con o sin token, y con el tipo de contenido correcto según el contexto.
...
// Devuelve → "2025-06-13 08:24:05"
export const getDateTime = (sprDate = "-", sprBwn = " ", sprTime = ":") => {
  let dt = new Date();

  //padStart(4, "0") para que cada número siempre tenga cierto número de dígitos.
  return (
...
//enviar datos tipo formulario
export const getFormData = (data) => {
  let form_data = new FormData();

  //Obtiene los nombres de las propiedades del objeto (data).
  //Por cada key, toma el valor data[key].
  Object.keys(data).forEach((key) => {
    ...
    } else { // Si es un File, lo agrega directamente y verifica null o vacio "".
      form_data.append(
        key,
        data[key] == null ||
...
//Convierte una cadena en base64 a un objeto Blob que puedes:Descargar como un archivo, Mostrar en pantalla (por ej. imágenes, PDFs).
export const getBlob = (b64, ext) => {
  const binary_string = window.atob(b64.replace(/\s/g, "")); // Elimina espacios en blanco
  const len = binary_string.length;
  let bytes = new Uint8Array(len);

  // Convierte la cadena base64 a un arreglo de bytes
  // Utiliza charCodeAt para obtener el valor numérico de cada carácter
  for (let i = 0; i < len; i++) {
    bytes[i] = binary_string.charCodeAt(i);
  }

  //Construye un Blob que representa el contenido binario en el tipo MIME que tú especifiques (application/pdf, application/zip, etc.).
  return new Blob([bytes.buffer], {
    type: "application/" + ext,
  });
};

//le pone a decimal un formato de moneda, con dos decimales.
...
//formatear un número como porcentaje, y si el valor es vacío o cero, te devuelve un guion.
